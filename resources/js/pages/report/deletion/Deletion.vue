...
export default {
  ...
  watch: {
    page: function () {
      this.fetchDeletionData();
    },
  },
  methods: {
    async fetchDeletionData() {
      ...
      const resultGetDeletionData = await this.$store.dispatch('callAPI', {
        url: '/deletionReportOauth',
        method: 'post',
        data: payload,
      });
      this.$store.commit('setShowProgressAPI', false);
      if (resultGetDeletionData.message === 'success') {
        this.data = resultGetDeletionData?.data?.data?.records || [];
        this.totalItems = resultGetDeletionData?.data?.data?.total || 0;
        this.tableLoaded = true;
        this.columns = resultGetDeletionData?.data?.data?.columns || [];
      } else {
        this.data = [];
        this.totalItems = 0;
        this.$swal({
          icon: 'error',
          text: this.getMessageResult(resultGetDeletionData),
        });
      }
