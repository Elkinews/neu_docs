<template>
  <div class="nde-table-userProfile-report">
  ...
  methods: {
    async fetchUserProfileData() {
      ...
      const payload = {
        ...
        active: this.dataFilter.active,
      };
      this.$store.commit('setShowProgressAPI', true);
      const resultGetUserProfileData = await this.$store.dispatch('callAPI', {
        url: '/userProfileReportOauth',
        method: 'post',
        data: payload,
      });
      this.$store.commit('setShowProgressAPI', false);
      if (resultGetUserProfileData.message === 'success') {
        this.data = resultGetUserProfileData?.data?.data?.records || [];
        this.totalItems = resultGetUserProfileData?.data?.data?.total || 0;
        this.tableLoaded = true;
        this.columns = resultGetUserProfileData?.data?.data?.columns || [];
      } else {
        this.data = [];
        this.totalItems = 0;
        this.$swal({
          icon: 'error',
          text: this.getMessageResult(resultGetUserProfileData),
        });
      }
    ...
    async pageExport() {
      ...
      await getUserProfileData(payload)
        .then((data) => {
