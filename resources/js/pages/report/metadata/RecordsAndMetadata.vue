...
export default {
  ...
  watch: {
    page: function () {
      this.fetchMetaDataReport();
    },
  },
  methods: {
    async fetchMetaDataReport() {
      ...
      const resultGetMetaDataReport = await this.$store.dispatch('callAPI', {
        url: '/recordMetadataReportOauth',
        method: 'post',
        data: payload,
      });
      this.$store.commit('setShowProgressAPI', false);
      if (resultGetMetaDataReport.message === 'success') {
        this.data = resultGetMetaDataReport?.data?.data?.records || [];
        this.totalItems = resultGetMetaDataReport?.data?.data?.total || 0;
        this.tableLoaded = true;
        this.columns = resultGetMetaDataReport?.data?.data?.columns || [];
      } else {
        this.data = [];
        this.totalItems = 0;
        this.$swal({
          icon: 'error',
          text: this.getMessageResult(resultGetMetaDataReport),
        });
      }
    ...
    async pageExport() {
      ...
      await getMetaDataReport(payload)
        .then((data) => {
