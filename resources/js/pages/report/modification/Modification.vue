...
export default {
  ...
  watch: {
    page: function () {
      this.fetchModificationData();
    },
  },
  methods: {
    async fetchModificationData() {
      ...
      const resultGetModificationData = await this.$store.dispatch('callAPI', {
        url: '/getModificationReportOauth',
        method: 'post',
        data: payload,
      });
      this.$store.commit('setShowProgressAPI', false);
      if (resultGetModificationData.message === 'success') {
        this.data = resultGetModificationData?.data?.data?.records || [];
        this.totalItems = resultGetModificationData?.data?.data?.total || 0;
        this.tableLoaded = true;
        this.columns = resultGetModificationData?.data?.data?.columns || [];
      } else {
        this.data = [];
        this.totalItems = 0;
        this.$swal({
          icon: 'error',
          text: this.getMessageResult(resultGetModificationData),
        });
      }
    ...
    async pageExport() {
      ...
      await getModificationData(payload)
        .then((data) => {
